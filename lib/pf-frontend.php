...
/*
				...
				if ( empty( $prdctfltr_global['active_filters'] ) && empty( $prdctfltr_global['active_taxonomies'] ) ) {
					self::$settings['cat_active'] = true;
				}
				else {
					 self::$settings['cat_active'] = false;
				}
		...
		public static function prdctfltr_wc_tax( $query, $that ) {
			...
			if ( isset( self::$settings['cat_active'] ) && self::$settings['cat_active'] === true ) {
	...
	/*										}*/
		...
		public static function prdctfltr_get_settings() {
			...
			if ( substr( $options['wc_settings_prdctfltr_loader'], 0, 4 ) !== 'css-' ) {
...
<?php
		...
		public static function get_filter( $type ) {
			...
			switch ( self::$filter['filter'] ) {
				case 'range' :
					self::get_dynamic_filter_title( 'range', self::$filter['taxonomy'], self::$settings['rng'], self::$filter['taxonomy'] == 'price' ? esc_html__( 'Price Range', 'prdctfltr' ) : self::$filter['taxonomy'] );
				break;
				case 'taxonomy' :
		...
		public static function get_true_filter( $filter ) {
			...
			switch( $filter ) {
				case 'per_page' :
					...
					self::$filter = array(
						'filter'        => $filter,
						'title'         => esc_html__( 'Products Per Page', 'prdctfltr' ),
				...
				case 'instock' :
					...
					self::$filter = array(
						'filter'        => $filter,
						'title'         => esc_html__( 'Product Availability', 'prdctfltr' ),
				...
				case 'price' :
					...
					self::$filter = array(
						'filter'        => $filter,
						'title'         => esc_html__( 'Price Range', 'prdctfltr' ),
